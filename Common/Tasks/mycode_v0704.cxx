...
struct MyAnalysisTask{
  ...
  void init(InitContext const&){
    ...
    histos.add("Correct", "Correct", kTH1F, {axisCounter});
    histos.add("Total", "Total", kTH2F, {axisCounter, axisEvent});
  }
               ...
               aod::MCPair const& truepairs)
  {
    if(collision.has_mcCollision()){
      for(auto& fwdtrack : fwdtracks){
        ...
        int mucolid, mcmuid, mckid, kpdg, estmuid, estkid, mumom, kmom, storemuid;
        float mupt, Col_z, mudcaX, mudcaY, mumftX, mumftY, mumftZ, kdcaX, kdcaY, kmftX, kmftY, kmftZ, mumcX, mumcY, mumcZ, kmcZ, estx, esty, estz;
          ...
          if(mfttrack.collisionId()==fwdColId && mfttrack.mcParticleId()==fwdtrack.mcParticleId()){
            ...
            for(auto& truepair : truepairs){
              if(truepair.muid()==mcmuid && truepair.colid()==mucolid){
                ...
                for(auto& mfttrack2 : mfttracks){
                  if(mfttrack2.has_collision() && mfttrack2.has_mcParticle()){
                    if(mfttrack2.collisionId()==mucolid && mfttrack2.mcParticleId()!=mcmuid){
                      auto mcParticle_mft2 = mfttrack2.mcParticle();
                      double mftchi2 = mfttrack2.chi2();
                      SMatrix5 mftpars(mfttrack2.x(), mfttrack2.y(), mfttrack2.phi(), mfttrack2.tgl(), mfttrack2.signed1Pt());
                      vector<double> mftv2;
                      ...
                      float pre_ky = kmftY + t*Ncy;
                      float pre_kz = kmftZ + t*Ncz;
                      float pcax = ((pre_mux+pre_kx)/2) - collision.posX();
                      float pcay = ((pre_muy+pre_ky)/2) - collision.posY();
                      float pcaz = ((pre_muz+pre_kz)/2) - collision.posZ();
                      float r_xyz = sqrt(pow(pre_kx-pre_mux, 2)+pow(pre_ky-pre_muy, 2)+pow(pre_kz-pre_muz, 2));
